...
**All secrets are managed in Azure Key Vault (`kvconfradar`)** for security and centralized management.

**Option A: Add Codespace Secret from Azure Key Vault (Recommended)**
1. Retrieve the OpenAI API key from Azure Key Vault:
   - Key Vault: `kvconfradar`
   - Azure Subscription ID: `8592e500-3312-4991-9d2a-2b97e43b1810`
   - Secret name: `CONFRADAR-SA-OPENAI` (service account) or `OPENAI-API-KEY`
2. Go to your [GitHub Settings → Codespaces](https://github.com/settings/codespaces)
3. Click **New secret**
4. Add the secret with one of these names:
   - `CONFRADAR_SA_OPENAI` (preferred for organization usage)
   - `OPENAI_API_KEY` (alternative, personal API key format: `sk-...`)
5. Select repository: `orgroman/confradar`
6. Restart your Codespace to apply

**Option B: Use Azure MCP in Codespace (For Azure-authenticated users)**
If you have Azure CLI access configured in your Codespace:
```bash
# Authenticate with Azure
az login

# Retrieve secret from Key Vault
az keyvault secret show --vault-name kvconfradar --name CONFRADAR-SA-OPENAI --query value -o tsv

# Set as environment variable (temporary, for current session)
export CONFRADAR_SA_OPENAI=$(az keyvault secret show --vault-name kvconfradar --name CONFRADAR-SA-OPENAI --query value -o tsv)
```

**Option C: Set in Terminal (Least secure, for testing only)**
```bash
...
> **Note:** Never commit secrets to the repository. Always use Azure Key Vault as the single source of truth for secrets.
